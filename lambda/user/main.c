...
#define STM32_FLASH_SIZE 256 	 		//??STM32?FLASH????(???K)
...
u16 Vccontrol_heat, CANID_base, CANID_Sent;
float temp_test_v, PT_VALUE, VF1, VF2, voltage_base;
float modbusbuffer1[8];
float modbusbuffer2[8];
uint16_t VcFault, VcReset;
u8 GPIO_Status[8], Vfindex_of_tables;
float SIFX_OUT;
__IO float  ADC13ConvertedVoltage = 0;
...
extern void UART_send_byte(uint8_t byte); //??1????
void ADC13_Init(void);
void set_voltage_output(float input_voltage);
...
int main(void)
{ 
	float lambdaMax=32.77f,heatpwmmax=60;
		float heat_pwm=1;
   u8 canbuf[8];
   u8 res;
   u8 key;
   u8 i, offset, address_eep, j, c = 0,spi_ns=0;
   u16 checksum, temp_value;
  RCC_ClocksTypeDef RCC_Clocks;
  __IO float InputVoltageMv = 0,Vch1 = 0, Vch2 = 0;
    TestStatus status;
    	 uint16_t  ADC1ConvertedValue = 0;
    	 uint16_t  ADC13ConvertedValue = 0;
  rxmsgisok = false;
  uarttransmit = false;
	Ua0=Ur0=0;
  ScWork_flow = 0;
  ...
  virtual_pwm1 = 1;
  virtual_periods1 = 1;
  voltage_base = 6.6; //initial voltage
  CANID_base = 1944;
   j = 0;
   for(i = 0; i < 14; i++)
  {
   ...
   for(i = 0; i < 14; i++)
  {
   ...
   temp_readback = SPIByte(0x7E00);
    ...
    /* Get ADC1 converted data */
         ADC13ConvertedValue =ADC_GetConversionValue(ADC1);
         
         /* Compute the voltage */
         ADC13ConvertedVoltage = 14.33 * ADC13ConvertedValue * 2.5 / 4096+0.31;

           GPIO_WriteBit(GPIOB,GPIO_Pin_0, 1);
           temp_readback = SPIByte(0x7800);
           delay(200);
				 
					spi_ns=(SPI_feedback & 0xFF)/64;
           if( spi_ns <2)
						 heat_pwm = 0;
...
#if APP
				Rpmes = (float)((u16)(UA + 32768)* 3.1/ 65535);//UA v
				Rpmes=g_cala[11]*Rpmes+g_calb[11];//ax+b
				//Rpmes=0.1*Rpmes+0.9*Ua0;
				//Ua0=Rpmes;
				
        Vfvalue_air = (float)((u16)(UR + 32768) * 2.5 / 65535 );//UR v
				...
				if(Vfair_fuel>32.77 || Vfair_fuel<0.1)
					Vfair_fuel=32.77;
				 
				RI = (float)((float)(Vfvalue_air - 0.2941 ) / (float)(2449) * 1000000);//RI 
				//control heat
				heatpwmmax=60*(12*12)/(ADC13ConvertedVoltage*ADC13ConvertedVoltage+0.01f);
				if(heatpwmmax>100)
					heatpwmmax=100;
				if(heatpwmmax<1)
					heatpwmmax=1;
                              if(Vccontrol_heat!= 0xFFFF)
                                 Vccontrol_heat++;
                              if( Vccontrol_heat <=1500)
                              {
                                 if((Vccontrol_heat % 100) == 0)
                                 {
                                    voltage_base = voltage_base + 0.15;
                                 }
                                 heat_pwm = 90 * (voltage_base * voltage_base)/(ADC13ConvertedVoltage*ADC13ConvertedVoltage+0.01f);
                              }
                              else
                              {
                              if(RI > 800)
   					...
   					heat_pwm+=0.005*(0.01*heatpwmmax);
   				else if(RI<220)
   					heat_pwm-=0.5;
   				else
   					heat_pwm-=0.01*(0.01*heatpwmmax);
   				if(heat_pwm<0)
   					heat_pwm=0;
   				
   				if(heat_pwm>heatpwmmax)
   					heat_pwm=heatpwmmax;
                              }
					if(RI>800)//enable
						Vfair_fuel= lambdaMax;
					/////////////////v out
					if(g_calb[13]!=0)
						temp_test_v=((g_cala[12]*g_cala[13]+g_calb[12]) / 5);//test
					else
					{
						if(Vfair_fuel<4)
							temp_test_v=((g_cala[12]*Vfair_fuel+g_calb[12]) / 5);
						else
							temp_test_v=((g_cala[12]*4+g_calb[12]) / 5);
					}
					if(temp_test_v>1)
						temp_test_v=1;
					if(temp_test_v<0)
						temp_test_v=0;
					Val= temp_test_v * 0xFFF;
					DAC_SetChannel1Data(DAC1, DAC_Align_12b_R, Val);
        //set_voltage_output(input_voltage);
/*	if(VfIpmeas > KfIpmeas[8])
		 ...
		 CANID_Sent = CANID_base + CANID_Merge * 3;
         ...
         outputdata[j+2] = ADC_float[2];
         outputdata[j+3] = ADC_float[3];
         CAN1_Send_Msg(CANID_Sent,&outputdata[0]);//1944
		 j = j + 4;
         ADC_float = (u8 *)(&heatpwmmax);//
         outputdata[j] = ADC_float[0];
         outputdata[j+1] = ADC_float[1];
         outputdata[j+2] = ADC_float[2];
         outputdata[j+3] = ADC_float[3];
         CAN1_Send_Msg((CANID_Sent + 1),&outputdata[8]);//1945
		 j = j + 4;
         ADC_float = (u8 *)(&heat_pwm);//
         outputdata[j] = ADC_float[0];
         outputdata[j+1] = ADC_float[1];
         outputdata[j+2] = ADC_float[2];
         outputdata[j+3] = ADC_float[3];
         ...
         outputdata[j+2] = ADC_float[2];
         outputdata[j+3] = ADC_float[3];
         CAN1_Send_Msg((CANID_Sent + 2),&outputdata[12]);//6]);//1946
		 j = j + 4;
       
/*
					outputdata[j] = 0;
					outputdata[j+1] =spi_ns;
					j = j + 2;
					CAN1_Send_Msg(1947,&outputdata[20]);//1947
*/
#endif
#ifdef CAL_APP
...
DMA_ITConfig(DMA2_Channel1, DMA_IT_TC, ENABLE);        //DMA??1??????
...
DMA_ITConfig(DMA1_Channel1, DMA_IT_TC | DMA_IT_HT, ENABLE);        //DMA??1??????
...
void Timer_Configuration_IC(void)
{
   ...
   TIM_SelectInputTrigger(TIM19, TIM_TS_TI2FP2);    //???????
   TIM_SelectInputTrigger(TIM19, TIM_TS_TI1FP1);    //???????
   TIM_SelectSlaveMode(TIM19, TIM_SlaveMode_Reset);//????  
   TIM_SelectMasterSlaveMode(TIM19, TIM_MasterSlaveMode_Enable); //??????????
   TIM_SelectSlaveMode(TIM12, TIM_SlaveMode_Reset);//????  
   TIM_SelectMasterSlaveMode(TIM12, TIM_MasterSlaveMode_Enable); //??????????
   TIM_Cmd(TIM19, ENABLE); 
   TIM_Cmd(TIM12, ENABLE); 
   TIM_ITConfig(TIM19, TIM_IT_CC1, ENABLE);         //????
   TIM_ITConfig(TIM19, TIM_IT_CC2, ENABLE);         //????
   TIM_ITConfig(TIM19, TIM_IT_CC3, ENABLE);         //????
   TIM_ITConfig(TIM19, TIM_IT_CC4, ENABLE);         //????
...
void set_voltage_output(float input_voltage)
{
   ...
   if(input_voltage < 5)
   {
      output_DC = input_voltage * 10 / 5;
      Output_voltage_count = Output_voltage_count + 1;
      if(Output_voltage_count > 10)
      {
         Output_voltage_count = 0;
         GPIO_ResetBits(GPIOC,GPIO_Pin_1);
      }
      else if(Output_voltage_count < output_DC)
      {
